...
from app.models.task import Task, TaskStage, TaskStatus, ActivityStatus
            ...
            for task in tasks:
                ...
                try:
                    ...
                    if should_end_activity:
                        ...
                        print(f"[WEBHOOK ACTIVITY LOG] Activity ended with status {activity_status.value}. Activity ID: {activity.id}, Task ID: {task.id}")
                    else:
...
async def github_webhook(
    ...
    # Handle GET/HEAD requests for webhook validation (GitHub sends these to verify the endpoint)
    if request.method in ["GET", "HEAD"]:
        print("=" * 60)
        print("✅ GitHub webhook validation request received (GET/HEAD)")
    ...
    try:
        ...
        print(f"   Signature: {'Present' if signature else 'Missing'}")
        ...
        if event_type == "workflow_run":
            ...
            if workflow_run:
                ...
                workflow_url = workflow_run.get("html_url", "")
                print(f"   [WORKFLOW_RUN] Workflow: {workflow_name}")
                print(f"   [WORKFLOW_RUN] Status: {workflow_status}, Conclusion: {workflow_conclusion}")
                print(f"   [WORKFLOW_RUN] URL: {workflow_url}")
        ...
        if repository:
            print(f"   [DEBUG] Repository object keys: {list(repository.keys())[:10]}")
        else:
            print(f"   [DEBUG] ⚠️  No repository object found in payload!")
        
        if not event_type:
            print("⚠️  WARNING: No X-GitHub-Event header found")
            return {
                "message": "Webhook received but no event type",
                "status": "warning"
            }
        ...
        print("[WEBHOOK] Creating notifications...")
        await notify_webhook_event(db, event_type, action, payload)
        
        # Handle specific event types
        if event_type == "pull_request":
            print("[WEBHOOK] Processing pull_request event...")
            await handle_pull_request_event(payload, db)
        elif event_type == "workflow_run":
            print("[WEBHOOK] Processing workflow_run event...")
            await handle_workflow_run_event(payload, db)
        elif event_type == "push":
            print("[WEBHOOK] Processing push event...")
            await handle_push_event(payload, db)
        else:
            print(f"[WEBHOOK] Event type '{event_type}' received but no specific handler")
...
async def handle_workflow_run_event(payload: dict, db: Session):
    ...
    workflow_url = workflow_run.get("html_url", "")
    head_branch = workflow_run.get("head_branch", "N/A")
    head_sha = workflow_run.get("head_sha", "N/A")
    repository = workflow_run.get("repository", {}) or payload.get("repository", {})
    ...
    print(f"[WORKFLOW_RUN HANDLER] Branch: {head_branch}, Commit: {head_sha[:7] if head_sha else 'N/A'}")
    print(f"[WORKFLOW_RUN HANDLER] URL: {workflow_url}")
    
    if conclusion == "failure":
        print(f"[WORKFLOW_RUN HANDLER] ⚠️  Workflow failed! Fetching detailed error information...")
        
        # Fetch detailed error information via GitHub API
        error_details = await fetch_workflow_error_details(repository, run_id)
        
        if error_details:
            print(f"[WORKFLOW_RUN HANDLER] Found {error_details['failed_count']} failed job(s):")
            for job in error_details["failed_jobs"]:
                print(f"  - Job: {job['name']}")
                print(f"    Failed steps: {len(job['failed_steps'])}")
                for step in job["failed_steps"]:
                    print(f"      • Step {step['number']}: {step['name']}")
        else:
            print(f"[WORKFLOW_RUN HANDLER] Could not fetch detailed errors (check GITHUB_TOKEN)")
            print(f"[WORKFLOW_RUN HANDLER] To identify the failed step:")
            print(f"  1. Open: {workflow_url}")
            print(f"  2. Click on the failed job (red ❌)")
            print(f"  3. Expand each step to see error details")
